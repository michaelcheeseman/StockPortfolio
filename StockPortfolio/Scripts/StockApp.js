        ...
        success: function (data) {
            ...
            $("#stockList").wijgrid({
                ...
                columns: [
                        ...
                        { dataKey: "Name" },
                        { dataKey: "Open", width: 200, dataType: "number", visible: true, dataParser: _numberParser },
                        { dataKey: "Close", headerText: "Prev Close", width: 200, dataType: "number", visible: true, dataParser: _numberParser },
                        { dataKey: "LastSale", headerText: "Last Sale", width: 200, dataType: "number", visible: true, dataParser: _numberParser },
...
$("#stockList").wijgrid("selection").selectedCells().item(0).value();
                ...
                cellStyleFormatter: function (args) {
                    var c = args.$cell, $btn;
                    if ((args.column.dataKey === "Open" ||
                        args.column.dataKey === "Close" ||
    ...
    $.ajax({
        ...
        success: function (data) {
            var changeText,
                ...
                bid = toStr(data.Bid),
                bidSize= toStr(data.BidSize),
                ask = toStr(data.Ask),
                askSize = toStr(data.AskSize),
                ...
                previousVolume = toStr(data.PreviousVolume, 0),
                avgVol = toStr(data.AverageDailyVolume, 0),
                settlement = toStr(data.Settlement, 0),
                previousSettlement = toStr(data.PreviousSettlement, 0),
            ...
            $("#summaryBid").text(bid);
            $("#summaryBidSize").text(bidSize);

            $("#summaryAsk").text(ask);
            $("#summaryAskSize").text(askSize);
            ...
            $("#summaryPreviousVolume").text(previousVolume);
            $("#summarySettlement").text(settlement);
            $("#summaryPreviousSettlement").text(previousSettlement);
            $("#corpName").text(name);
...
stockApp.updateGrid = function (data) {
    $.each(data, function (i, n) {
        ...
        var symbol = n.Symbol;
        stockApp.updateCell(symbol, "Open", n);
        stockApp.updateCell(symbol, "Close", n);
